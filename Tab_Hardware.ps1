...
$btnBatDauQuet.Add_Click({
    ChayTacVu "Đang quét phần cứng" {
        $os = Get-CimInstance Win32_OperatingSystem; $cs = Get-CimInstance Win32_ComputerSystem; $mb = Get-CimInstance Win32_BaseBoard; $bios = Get-CimInstance Win32_BIOS; $cpu = Get-CimInstance Win32_Processor
        $info =  "================ HỆ THỐNG ================`r`n"
        $info += "Hệ điều hành: $($os.Caption) (Bản $($os.BuildNumber))`r`nMáy:          $($cs.Manufacturer) - $($cs.Model)`r`n"
        $info += "`r`n================ MAINBOARD ================`r`nHãng:     $($mb.Manufacturer)`r`nModel:    $($mb.Product)`r`nSerial:   $($mb.SerialNumber)`r`nBIOS:     $($bios.SMBIOSBIOSVersion) ($($bios.ReleaseDate.ToString('dd/MM/yyyy')))`r`n"
        $info += "`r`n================ CPU (VI XỬ LÝ) ================`r`nTên:      $($cpu.Name)`r`nSocket:   $($cpu.SocketDesignation)`r`nTốc độ:   $([math]::Round($cpu.MaxClockSpeed/1000, 2)) GHz`r`nNhân/L:   $($cpu.NumberOfCores) Nhân / $($cpu.NumberOfLogicalProcessors) Luồng`r`n"
        $memArr = Get-CimInstance Win32_PhysicalMemoryArray; $dimms = Get-CimInstance Win32_PhysicalMemory
        function LayLoaiRam($c) { switch($c){20{"DDR"} 21{"DDR2"} 24{"DDR3"} 26{"DDR4"} 34{"DDR5"} Default{"Không rõ"}} }
        $info += "`r`n================ RAM (BỘ NHỚ) ================`r`nKhe cắm:  $(@($dimms).Count) dùng / $($memArr.MemoryDevices) tổng`r`n"
        foreach ($d in $dimms) { $cap = [math]::Round($d.Capacity/1GB, 0); $info += "- Slot $($d.DeviceLocator): ${cap}GB $(LayLoaiRam $d.SMBIOSMemoryType) ($($d.Speed)MHz) | Hãng: $($d.Manufacturer) | P/N: $($d.PartNumber.Trim())`r`n" }
        $info += "`r`n================ Ổ CỨNG ================`r`n"
        foreach ($d in Get-CimInstance Win32_DiskDrive) {
            $ws = [math]::Round($d.Size/1GB, 0); $ms = [math]::Round($d.Size/1000000000, 0); $lblM = if($ms -ge 1000){"$([math]::Round($ms/1000,0)) TB"}else{"$ms GB"}
